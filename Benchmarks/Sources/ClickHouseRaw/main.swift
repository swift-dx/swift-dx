...
// the synchronous POSIX-socket RawClickHouseConnection — no NIO, no
...
nonisolated(unsafe) private let connection: RawClickHouseConnection = {
    do {
        return try RawClickHouseConnection(host: host, port: port, user: user, password: password, database: database)
    } catch {
