...
// ClickHouseConnectionPool (16 connections) and run a random mix of
...
@Suite(
    "DXClickHouse stability — concurrency stress (100 Tasks × shared 16-conn pool)",
...
struct ClickHouseConcurrencyStressTests {
    ...
    func testHundredTasksOverSharedPoolNoLeaks() async throws {
        let maxConnections = ClickHouseStabilitySupport.concurrencyPoolMaxConnections
        let taskCount = ClickHouseStabilitySupport.concurrencyStressTaskCount
        ...
        let table = ClickHouseStabilitySupport.uniqueTable(prefix: "conc")
        ...
        let durationSeconds = ClickHouseStabilitySupport.concurrencyStressDurationSeconds
    ...
    private static func seedFixture(pool: ClickHouseConnectionPool, table: String) async throws {
        try await pool.withConnection { connection in
    ...
    private static func runOneTask(
        ...
        var random = ClickHouseStabilityRandom(seed: UInt64(taskIndex + 1) &* 2_654_435_761)
