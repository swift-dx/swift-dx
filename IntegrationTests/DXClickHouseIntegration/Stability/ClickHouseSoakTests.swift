...
// Sustained-load soak for DXClickHouse. Drives every operation
// surface on ClickHouseClient through a single shared client for
...
//   * scalar:  ClickHouseClient.scalar([UInt8], as:)
//   * select:  ClickHouseClient.select(_, as:) AsyncThrowingStream
//   * insert:  ClickHouseClient.insert(into:rows:) with Codable
//   * stream:  ClickHouseClient.stream(_, as:, handler:) with the
...
struct ClickHouseSoakTests {
    ...
    func testFiveMinuteSoakHoldsBoundedRSS() async throws {
        let client = try await ClickHouseStabilitySupport.makeClient()
        defer { Task { await client.close() } }

        let table = ClickHouseStabilitySupport.uniqueTable(prefix: "soak")
        ...
        let baselineResident = ClickHouseStabilityProcessRSS.currentBytes()
        var peakResident = baselineResident
        var insertCounter: UInt64 = UInt64(Self.warmupRows)

        let durationSeconds = ClickHouseStabilitySupport.soakDurationSeconds
        let soakStart = ContinuousClock.now
        let deadline = soakStart.advanced(by: .seconds(durationSeconds))

        var windows: [ClickHouseStabilityWindow] = []
        var currentWindow = ClickHouseStabilityWindow()
        ...
        while ContinuousClock.now < deadline {
            ...
            do {
                ...
                currentWindow.record(microseconds: ClickHouseStabilitySupport.microsecondsSince(operationStart))
                totalSamples += 1
            } catch {
        ...
        windows.append(currentWindow)
        let finalResident = ClickHouseStabilityProcessRSS.currentBytes()
    ...
    private static func warmFixture(client: ClickHouseClient, table: String) async throws {
