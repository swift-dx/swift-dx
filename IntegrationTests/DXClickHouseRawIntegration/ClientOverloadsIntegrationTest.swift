...
// RawClickHouseClient: execute, ping, scalar, select, insert, stream,
// each in every input form (raw `[UInt8]` SQL, Foundation Codable,
// Sequence, AsyncSequence, callback, DXMessageHandler).
@Suite(
    "DXClickHouseRaw client overload integration",
    .enabled(if: ProcessInfo.processInfo.environment["CH_INTEGRATION_HOST"] != nil)
)
struct RawClickHouseClientOverloadIntegration {
    ...
    actor CollectingHandler: DXMessageHandler {
        ...
        func snapshot() -> (rows: [OrderRow], failures: [RawClickHouseError]) {
            (rows, failures)
        }
    }
    ...
    func pingCallback() async throws {
        let client = try await Self.makeClient()
        defer { Task { await client.close() } }
        let outcome: RawClickHouseError? = await withCheckedContinuation { continuation in
    ...
    func scalarCallback() async throws {
        let client = try await Self.makeClient()
        defer { Task { await client.close() } }
        let outcome: Result<UInt64, RawClickHouseError> = await withCheckedContinuation { continuation in
    ...
    func selectCallback() async throws {
        ...
        defer { Task { await client.close() } }
        struct IDRow: Decodable, Sendable, Equatable { let id: UInt64 }
        let outcome: Result<[IDRow], RawClickHouseError> = await withCheckedContinuation { continuation in
    ...
    func insertCallback() async throws {
        ...
        try await client.execute("""
            ...
            """)
        let outcome: Result<RawClickHouseInsertSummary, RawClickHouseError> = await withCheckedContinuation { continuation in
    ...
    private static func makeClient() async throws -> RawClickHouseClient {
