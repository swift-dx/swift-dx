...
//
//   * RawClickHouseClient.insert<T>(into:rows:)
//   * RawClickHouseClient.select<T>(_:as:settings:parameters:)
//   * RawClickHouseClient.scalar<T>(_:as:)
//
...
@Suite(
    "DXClickHouseRaw Codable round-trip integration",
    .enabled(if: ProcessInfo.processInfo.environment["CH_INTEGRATION_HOST"] != nil)
)
struct RawClickHouseCodableRoundTripIntegration {
    ...
    func selectUnknownTableSurfacesQueryFailed() async throws {
        let client = try await Self.makeClient()
        defer { Task { await client.close() } }
        let caught: RawClickHouseError? = await captureClientError {
    ...
    private static func runDDL(client: RawClickHouseClient, _ sql: String) async throws {
        let conn = try RawClickHouseConnection(
            host: host, port: port,
            user: user, password: password, database: database
        )
        defer { conn.close() }
...
private func captureClientError(_ body: () async throws -> Void) async -> RawClickHouseError? {
    do {
        try await body()
        return nil
