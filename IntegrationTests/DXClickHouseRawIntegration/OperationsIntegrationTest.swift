...
@Suite(
    "DXClickHouseRaw integration: production operation surface",
    .enabled(if: ProcessInfo.processInfo.environment["CH_INTEGRATION_HOST"] != nil)
)
struct RawClickHouseOperationsIntegration {
    ...
    private static func connect() throws -> RawClickHouseConnection {
        try RawClickHouseConnection(
            host: host,
            port: port,
            user: user,
            password: password,
    ...
    func settingsRoundtrip() throws {
        ...
        let settings = RawClickHouseQuerySettings([
            RawClickHouseQuerySetting(name: "max_threads", value: "1"),
        ])
    ...
    func settingsUnknownRejected() throws {
        ...
        defer { connection.close() }
        let settings = RawClickHouseQuerySettings([
            RawClickHouseQuerySetting(name: "this_setting_does_not_exist_anywhere", value: "1", important: true),
        ])
    ...
    func parameterisedQuery() throws {
        ...
        let parameters = RawClickHouseQueryParameters([
            RawClickHouseQueryParameter(name: "value", value: "'1729'"),
        ])
        try connection.sendQuery(
            "SELECT {value:UInt64}",
    ...
    @Test("Malformed SQL surfaces a typed RawClickHouseServerException")
    ...
    func progressCallbackFires() throws {
        ...
        let settings = RawClickHouseQuerySettings([
            ...
            RawClickHouseQuerySetting(name: "interactive_delay", value: "100000"),
        ])
        try connection.sendQuery(
    ...
    func profileInfoCallback() throws {
        ...
        var profileInfo = RawClickHouseProfileInfo(
    ...
    func queryIDInLog() throws {
        ...
        _ = try connection.receiveBlocks { _, _ in }
        let parameters = RawClickHouseQueryParameters([
            RawClickHouseQueryParameter(name: "qid", value: "'\(queryID)'"),
        ])
        try connection.sendQuery(
...
// progress callbacks).
@Suite(
    "DXClickHouseRaw integration: async operation surface",
    .enabled(if: ProcessInfo.processInfo.environment["CH_INTEGRATION_HOST"] != nil)
)
struct AsyncRawClickHouseOperationsIntegration {
    ...
    private static func connect() async throws -> AsyncRawClickHouseConnection {
    ...
    func asyncParameterisedQuery() async throws {
        ...
        try await connection.sendQuery(
            ...
            parameters: RawClickHouseQueryParameters([
                RawClickHouseQueryParameter(name: "value", value: "'12345'"),
            ])
        )
...
@Suite(
    "DXClickHouseRaw integration: pool features",
    .enabled(if: ProcessInfo.processInfo.environment["CH_INTEGRATION_HOST"] != nil)
)
struct RawClickHouseConnectionPoolFeaturesIntegration {
    ...
    func multiEndpointFailover() async throws {
        let configuration = RawClickHouseConnectionPool.Configuration(
            endpoints: [
                ...
                RawEndpoint(host: Self.host, port: Self.port),
            ],
            user: Self.user,
