...
//
//   * The injected fault surfaces as a typed RawClickHouseError (or
//     RawClickHouseConnectionPool.Failure for the pool-exhaustion
...
@Suite(
    "DXClickHouseRaw stability — fault injection (7 production-shape scenarios)",
    .enabled(if: ProcessInfo.processInfo.environment["CH_INTEGRATION_HOST"] != nil),
    .serialized
)
struct RawFaultInjectionTests {
    ...
    func killAndRestartContainerMidQuery() async throws {
        ...
        let connection = try RawClickHouseConnection(
            host: RawStabilitySupport.host,
            port: RawStabilitySupport.port,
            user: RawStabilitySupport.user,
            password: RawStabilitySupport.password,
            database: RawStabilitySupport.database,
        ...
        let fdBaseline = RawStabilityFileDescriptorCount.currentCount()
        ...
        if fdBaseline > 0 {
            let fdAfter = RawStabilityFileDescriptorCount.currentCount()
    ...
    func midStreamTaskCancellation() async throws {
        let connection = try await RawStabilitySupport.makeAsyncConnection()
        ...
        let elapsedMicroseconds = RawStabilitySupport.microsecondsSince(cancellationStart)
    ...
    func connectTimeoutToWrongPortSurfacesTypedError() async throws {
        ...
        let elapsedMicroseconds = RawStabilitySupport.microsecondsSince(start)
    ...
    func poolExhaustionSurfacesTypedTimeout() async throws {
        ...
        do {
            ...
            holderDone.wait()
            _ = holder
            Issue.record("expected RawClickHouseConnectionPool.Failure, got untyped \(error)")
            return
        }
        let elapsedMicroseconds = RawStabilitySupport.microsecondsSince(acquireStart)
        holderDone.wait()
        _ = holder
    ...
    func serverSideQueryErrorSurfacesQueryFailed() async throws {
        ...
        do {
            ...
            Issue.record("expected typed RawClickHouseError for server-side error, got \(error)")
            return
        }
    ...
    func tcpResetMidReceiveSurfacesTypedError() async throws {
        let forwarder = RawStabilityResettingForwarder(
            upstreamHost: RawStabilitySupport.host,
            upstreamPort: RawStabilitySupport.port
        )
        let proxyPort: Int
        do {
        ...
        #expect(untyped == nil, "expected typed RawClickHouseError for TCP RST, got \(untyped ?? "")")
        guard let error = captured else {
        ...
        let recovered = try await RawStabilitySupport.makeAsyncConnection()
    ...
    func thousandConnectDisconnectCyclesLeakNoDescriptors() async throws {
        ...
        let baseline = RawStabilityFileDescriptorCount.currentCount()
        let cycles = 1_000
        for _ in 0..<cycles {
            let connection = try await RawStabilitySupport.makeAsyncConnection()
            await connection.close()
        }
        let after = RawStabilityFileDescriptorCount.currentCount()
...
// stream.
final class RawStabilityResettingForwarder: @unchecked Sendable {
    ...
    func bind() throws -> Int {
        ...
        if listenFd < 0 {
            throw NSError(domain: "RawStabilityForwarder", code: Int(errno), userInfo: nil)
        }
        var reuse: Int32 = 1
        ...
        if bindResult < 0 {
            ...
            close(listenFd)
            throw NSError(domain: "RawStabilityForwarder", code: Int(savedErrno), userInfo: nil)
        }
