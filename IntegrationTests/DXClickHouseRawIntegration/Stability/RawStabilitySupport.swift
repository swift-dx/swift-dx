...
// DXClickHouseRaw. Nesting the suites inside `Stability` lets a single
// `--filter DXClickHouseRawIntegration.Stability` regex select all
...
// IntegrationTests/DXClickHouseRawIntegration/Stability/. Mirrors the
// DXClickHouse soak helpers but targets DXClickHouseRaw and reads its
...
enum RawStabilitySupport {
    ...
    static func makeAsyncConnection() async throws -> AsyncRawClickHouseConnection {
    ...
    static func makeClient() async throws -> RawClickHouseClient {
        try await RawClickHouseClient(
            host: host,
            port: port,
            user: user,
            password: password,
    ...
    ) async throws -> RawClickHouseConnectionPool {
...
// surfaces.
struct RawStabilityRandom: RandomNumberGenerator {
...
struct RawStabilityWindow: Sendable {
    ...
    func p99Microseconds() -> Int64 {
        var sorted = samples
        sorted.sort()
        return RawStabilitySupport.percentileMicroseconds(sorted, 0.99)
    }

    func p95Microseconds() -> Int64 {
        var sorted = samples
        sorted.sort()
        return RawStabilitySupport.percentileMicroseconds(sorted, 0.95)
    }

    func p50Microseconds() -> Int64 {
        var sorted = samples
        sorted.sort()
        return RawStabilitySupport.percentileMicroseconds(sorted, 0.50)
    }
}
...
// short-circuits.
enum RawStabilityFileDescriptorCount {
