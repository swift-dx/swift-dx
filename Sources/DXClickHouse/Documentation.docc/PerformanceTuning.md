...
How to extract the throughput `DXClickHouse` is designed for. Covers
...
let settings = ClickHouseQuerySettings([
    ClickHouseQuerySetting(name: "max_block_size", value: "65536"),
    ClickHouseQuerySetting(name: "max_threads", value: "4"),
    ClickHouseQuerySetting(name: "output_format_parallel_formatting", value: "0"),
])
```
...
Use ``ClickHouseQueryParameters`` for runtime values:

```swift
let parameters = ClickHouseQueryParameters([
    ClickHouseQueryParameter(name: "since", value: "2025-01-01"),
    ClickHouseQueryParameter(name: "kind", value: "purchase"),
])
...
5. **Pool waits.** Check ``ClickHouseConnectionPool/Stats/waiters``
   and ``ClickHouseConnectionPool/Stats/acquireTimeouts``. A
