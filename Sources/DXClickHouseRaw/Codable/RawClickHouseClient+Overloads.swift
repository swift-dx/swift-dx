...
// Public input-form overloads for RawClickHouseClient. Every operation
...
extension RawClickHouseClient {

    public func execute(_ sql: String) async throws(RawClickHouseError) {
        try await executeAndDrain(sql: sql)
    }

    public func execute(_ sqlBytes: [UInt8]) async throws(RawClickHouseError) {
        try await executeAndDrain(sql: Self.decodeSQL(sqlBytes))
    }

    public nonisolated func execute(_ sql: String, completion: @escaping DXCallback<Void, RawClickHouseError>) {
        Task { [self] in
            do {
    ...
    public nonisolated func ping(completion: @escaping DXCallback<Void, RawClickHouseError>) {
        Task { [self] in
            do {
                try await self.ping()
                completion(.success(()))
    ...
    public func ping() async throws(RawClickHouseError) {
        try await pingDrain()
    }
    ...
    public nonisolated func scalar<T: Decodable & Sendable>(
        _ sql: String,
        as type: T.Type,
        completion: @escaping DXCallback<T, RawClickHouseError>
    ) {
        Task { [self] in
            do {
    ...
    public nonisolated func stream<T: Decodable & Sendable, Handler: DXMessageHandler<T, RawClickHouseError>>(
        _ sql: String,
        as type: T.Type,
        handler: Handler
    ) -> Task<Void, Never> {
    ...
    public func insert<S: Sequence & Sendable>(
        into table: String,
        rows: S
    ) async throws(RawClickHouseError) -> RawClickHouseInsertSummary where S.Element: Encodable & Sendable {
    ...
    public func insert<Source: AsyncSequence & Sendable, Row: Encodable & Sendable>(
        into table: String,
        rows: Source
    ) async throws(RawClickHouseError) -> RawClickHouseInsertSummary where Source.Element == Row {
    ...
    public nonisolated func insert<T: Encodable & Sendable>(
        into table: String,
        rows: [T],
        completion: @escaping DXCallback<RawClickHouseInsertSummary, RawClickHouseError>
    ) {
        Task { [self] in
            do {
    ...
    private func executeAndDrain(sql: String) async throws(RawClickHouseError) {
        try await runOnTransport { transport throws(RawClickHouseError) in
            try transport.connection.sendQuery(sql)
            _ = try transport.connection.receiveBlocks { _, _ in }
        }
    }

    private func pingDrain() async throws(RawClickHouseError) {
        try await runOnTransport { transport throws(RawClickHouseError) in
    ...
    private func sendPreEncodedInsert(
        ...
        let writtenCounters = try await runOnTransportReturning { transport throws(RawClickHouseError) -> (UInt64, UInt64) in
    ...
    private static func collectRows<T: Sendable>(
        ...
        let outcome: Result<[T], RawClickHouseError> = await drainStream(stream: stream)
        switch outcome {
    ...
    private static func toTypedError(error: Error, stage: String) -> RawClickHouseError {
        switch error {
        case let typed as RawClickHouseError: return typed
    ...
    private static func materialise<Source: AsyncSequence & Sendable, Row: Encodable & Sendable>(
        rows: Source
    ) async throws(RawClickHouseError) -> [Row] where Source.Element == Row {
        var collected: [Row] = []
        do {
    ...
    private static func pumpStream<T: Decodable & Sendable, Handler: DXMessageHandler<T, RawClickHouseError>>(
    ...
    private static func forwardRows<T: Decodable & Sendable, Handler: DXMessageHandler<T, RawClickHouseError>>(
        upstream: AsyncThrowingStream<T, Error>,
        handler: Handler
    ) async throws {
    ...
    private func runOnTransport(
        _ body: @escaping @Sendable (ClientTransportBox) throws(RawClickHouseError) -> Void
    ) async throws(RawClickHouseError) {
        _ = try await runOnTransportReturning { transport throws(RawClickHouseError) -> Int in
            try body(transport)
            return 0
        }
    }
    ...
    nonisolated func runOnTransportReturning<Value: Sendable>(
        ...
        let outcome: Result<Value, RawClickHouseError> = await dispatchOnWorker(body: body)
        switch outcome {
    ...
    private nonisolated func dispatchOnWorker<Value: Sendable>(
        body: @escaping @Sendable (ClientTransportBox) throws(RawClickHouseError) -> Value
    ) async -> Result<Value, RawClickHouseError> {
    ...
    private static func runBody<Value: Sendable>(
        body: @Sendable (ClientTransportBox) throws(RawClickHouseError) -> Value,
        transport: ClientTransportBox
    ) -> Result<Value, RawClickHouseError> {
        do {
