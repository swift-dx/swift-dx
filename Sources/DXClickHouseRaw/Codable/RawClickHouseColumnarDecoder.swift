...
struct RawClickHouseColumnarDecoder: Decoder {
    ...
    func container<Key>(keyedBy type: Key.Type) throws -> KeyedDecodingContainer<Key> where Key: CodingKey {
        KeyedDecodingContainer(RawClickHouseColumnarKeyedDecodingContainer<Key>(
...
final class RawClickHouseColumnarDecoderState {

    let columns: [RawClickHouseNamedColumn]
    let columnIndexByName: [String: Int]
    var rowIndex: Int = 0

    init(columns: [RawClickHouseNamedColumn]) {
