...
struct RawClickHouseColumnarKeyedDecodingContainer<Key: CodingKey>: KeyedDecodingContainerProtocol {

    let state: RawClickHouseColumnarDecoderState
    ...
    private func column(forKey key: Key) throws -> RawClickHouseTypedColumn {
        guard let slot = state.slot(for: key.stringValue) else {
            throw missing(key)
        }
    ...
    private func requirePresent<T>(_ value: RawClickHouseNullable<T>, key: Key) throws -> T {
        switch value {
