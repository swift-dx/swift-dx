...
// into the columnar `[RawClickHouseNamedColumn]` representation the
...
public final class RawClickHouseRowEncoder: Sendable {

    public init() {}

    public func encode<T: Encodable & Sendable>(_ rows: [T]) throws(RawClickHouseError) -> [RawClickHouseNamedColumn] {
        let storage = RawClickHouseRowEncoderStorage()
        do {
            for row in rows {
                storage.beginRow()
                let encoder = RawClickHouseRowEncoderImpl(storage: storage)
            ...
            throw .protocolError(stage: "RawClickHouseRowEncoder", message: "\(error)")
        }
...
struct RawClickHouseRowEncoderImpl: Encoder {
    ...
    func container<Key>(keyedBy type: Key.Type) -> KeyedEncodingContainer<Key> where Key: CodingKey {
        KeyedEncodingContainer(RawClickHouseRowKeyedContainer<Key>(storage: storage, codingPath: codingPath))
    }

    func unkeyedContainer() -> UnkeyedEncodingContainer {
        RawClickHouseRowRejectingContainer(codingPath: codingPath)
    }
