...
// RawClickHouseRowEncoderStorage.
struct RawClickHouseRowKeyedContainer<Key: CodingKey>: KeyedEncodingContainerProtocol {

    let storage: RawClickHouseRowEncoderStorage
    var codingPath: [CodingKey]

    init(storage: RawClickHouseRowEncoderStorage, codingPath: [CodingKey]) {
    ...
    mutating func superEncoder() -> Encoder {
        RawClickHouseRowRejectingEncoder(codingPath: codingPath)
    }

    mutating func superEncoder(forKey key: Key) -> Encoder {
        RawClickHouseRowRejectingEncoder(codingPath: codingPath + [key])
    }

    @inline(__always)
    private func toNullable<Wrapped: Sendable>(_ value: Wrapped?) -> RawClickHouseNullable<Wrapped> {
