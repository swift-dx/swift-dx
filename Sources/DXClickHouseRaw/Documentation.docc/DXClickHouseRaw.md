...
`DXClickHouseRaw` is the lower of the two ClickHouse clients in SwiftDX.
...
- A synchronous transport (`RawClickHouseConnection`) that owns a POSIX
  ...
  is exhausted.
- An async actor (`AsyncRawClickHouseConnection`) that owns a private
...
- A typed Codable façade (``RawClickHouseClient``) plus a connection
  pool (``RawClickHouseConnectionPool``) for production traffic. The
...
> Important: `DXClickHouseRaw` is experimental. The wire transport
...
- ``RawClickHouseClient``
- ``RawClickHouseConnectionPool``
- ``AsyncRawClickHouseConnection``
- ``RawClickHouseConnection``
- ``RawEndpoint``
- ``ReconnectionPolicy``
...
- ``RawClickHouseQuerySetting``
- ``RawClickHouseQueryParameters``
- ``RawClickHouseQueryParameter``

### Receiving results

- ``RawClickHouseProgress``
- ``RawClickHouseProfileInfo``
- ``RawClickHouseProfileEvents``
- ``RawClickHouseInsertSummary``
- ``RawClickHouseTypedColumn``
- ``RawClickHouseNamedColumn``
- ``RawClickHouseNullable``

### Errors

- ``RawClickHouseError``
- ``RawClickHouseServerException``
- ``RawEndpointFailure``
...
Reach for `DXClickHouseRaw` when one of the following is true:
