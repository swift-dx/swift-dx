...
Every operation on ``RawClickHouseClient`` that accepts SQL or row data
...
`DXClickHouseRaw` follows the same overload pattern used across
...
the public surface. `DXClickHouseRaw` is built precisely to avoid
...
Both shapes accept optional ``RawClickHouseQuerySettings`` and
...
The returned `RawClickHouseInsertSummary.rowsSent` is zero on this
