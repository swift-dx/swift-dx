...
public struct RawClickHouseQueryParameter: Sendable, Equatable {
...
public struct RawClickHouseQueryParameters: Sendable, Equatable {

    public static let empty = RawClickHouseQueryParameters([])

    static let revisionWithQueryParameters: UInt64 = 54_459

    public let entries: [RawClickHouseQueryParameter]

    public init(_ entries: [RawClickHouseQueryParameter] = []) {
    ...
    func encode(into output: inout [UInt8], revision: UInt64) throws(RawClickHouseError) {
        ...
        for entry in entries {
            ...
            RawClickHouseWire.writeUVarInt(RawClickHouseQuerySettings.flagCustom, into: &output)
