...
// Production-grade pool of AsyncRawClickHouseConnection instances.
//
// Capability summary:
//
...
//     `RawClickHouseError.endpointsExhausted` surface to the caller.
//
...
public actor RawClickHouseConnectionPool {
    ...
    public struct Configuration: Sendable {
        ...
        public let endpoints: [RawEndpoint]
        ...
        // open fail with `RawClickHouseError.endpointsExhausted`.
        public init(
        ...
        public init(
            ...
            self.init(
                endpoints: [RawEndpoint(host: host, port: port)],
                user: user,
                password: password,
                database: database,
    ...
    public enum Failure: Error, Sendable, Equatable, CustomStringConvertible {
        ...
        case allEndpointsFailed(failures: [RawEndpointFailure])
    ...
    private func preflight(connection: AsyncRawClickHouseConnection) async -> Bool {
        do {
    ...
    private func openWithFailover() async throws -> PooledConnection {
        var failures: [RawEndpointFailure] = []
        for _ in 0..<configuration.endpoints.count {
            ...
            do {
                ...
                if case .openFailed(let reason) = failure {
                    ...
                    failures.append(RawEndpointFailure(host: endpoint.host, port: endpoint.port, reason: reason))
                    endpointFailovers += 1
                    continue
                }
                throw failure
                ...
                let endpoint = configuration.endpoints[index]
                failures.append(RawEndpointFailure(host: endpoint.host, port: endpoint.port, reason: String(describing: error)))
                endpointFailovers += 1
                continue
            }
        }
