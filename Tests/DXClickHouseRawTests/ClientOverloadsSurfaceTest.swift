...
// `RawClickHouseClient` is wired up and reachable through the
...
// renamed or removed.
@Suite("RawClickHouseClient overload surface")
struct RawClickHouseClientOverloadSurface {
    ...
    final class CapturingHandler: DXMessageHandler, @unchecked Sendable {
        ...
        var errors: [RawClickHouseError] = []

        func receive(_ message: DemoRow) async { received.append(message) }
        func receive(error: RawClickHouseError) async { errors.append(error) }
    }

    @Test("DXCallback typealias resolves to a result-bearing closure")
    func dxCallbackTypealias() {
        let callback: DXCallback<Int, RawClickHouseError> = { result in
            switch result {
    ...
    @Test("DXMessageHandler conformance compiles for RawClickHouseError")
    func dxMessageHandlerConformance() async {
        let handler = CapturingHandler()
        await handler.receive(DemoRow(id: 1, name: "alice"))
        await handler.receive(error: RawClickHouseError.protocolError(stage: "test", message: "demo"))
    ...
    @Test("Every overload signature is present on RawClickHouseClient")
    func overloadSignaturesExist() {
        let exercise: @Sendable (RawClickHouseClient) async throws -> Void = { client in
...
struct SequenceWrapper: Sequence, Sendable {
    ...
    let rows: [RawClickHouseClientOverloadSurface.DemoRow]

    func makeIterator() -> Array<RawClickHouseClientOverloadSurface.DemoRow>.Iterator {
        rows.makeIterator()
    }
}
