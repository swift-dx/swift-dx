...
@Suite("DXClickHouseRaw Codable decoder")
struct RawClickHouseColumnarDecoderTest {
    ...
    func decodesBasicRows() throws {
        let columns = [
            RawClickHouseNamedColumn(name: "id", column: .uint64([100, 200])),
            RawClickHouseNamedColumn(name: "name", column: .string(["alpha", "beta"])),
            RawClickHouseNamedColumn(name: "score", column: .float64([1.5, 2.5])),
        ]
        let rows: [Row] = try RawClickHouseCodableDecoder.decodeRows(
            type: Row.self,
            columns: columns,
            rowCount: 2
        )
    ...
    func decodesNullableColumns() throws {
        let columns = [
            RawClickHouseNamedColumn(name: "id", column: .uint64([1, 2])),
            RawClickHouseNamedColumn(name: "label", column: .nullableString([.present("hello"), .absent])),
            RawClickHouseNamedColumn(name: "count", column: .nullableInt32([.present(42), .absent])),
        ]
        let rows: [OptRow] = try RawClickHouseCodableDecoder.decodeRows(
            type: OptRow.self,
            columns: columns,
            rowCount: 2
        )
    ...
    func missingColumnError() throws {
        let columns = [
            RawClickHouseNamedColumn(name: "id", column: .uint64([1])),
        ]
        let caught = captureDecoderError {
            _ = try RawClickHouseCodableDecoder.decodeRows(type: Row.self, columns: columns, rowCount: 1)
        }
        switch caught {
        case .some(let error):
            switch error {
    ...
    func nullIntoNonOptional() throws {
        let columns = [
            RawClickHouseNamedColumn(name: "id", column: .uint64([1])),
            RawClickHouseNamedColumn(name: "name", column: .nullableString([.absent])),
            RawClickHouseNamedColumn(name: "score", column: .float64([1.0])),
        ]
        let caught = captureDecoderError {
            _ = try RawClickHouseCodableDecoder.decodeRows(type: Row.self, columns: columns, rowCount: 1)
        }
    ...
    private func captureDecoderError(_ body: () throws -> Void) -> RawClickHouseError? {
        do {
            try body()
            return nil
...
@Suite("DXClickHouseRaw block writer + decoder round-trip")
struct RawClickHouseBlockRoundTripTest {
    ...
    func scalarRoundTrip() throws {
        ...
        let dataPacket = try RawClickHouseBlockWriter.encodeDataPacket(columns: columns, revision: 54_478)
        ...
        let recovered: [Row] = try RawClickHouseCodableDecoder.decodeRows(
    ...
    func dateRoundTrip() throws {
        ...
        let dataPacket = try RawClickHouseBlockWriter.encodeDataPacket(columns: columns, revision: 54_478)
        let parsed = try Self.parseEncodedDataPacket(dataPacket)
        let recovered: [DateRow] = try RawClickHouseCodableDecoder.decodeRows(
    ...
    func uuidRoundTrip() throws {
        ...
        let columns = try RawClickHouseRowEncoder().encode(inputRows)
        let dataPacket = try RawClickHouseBlockWriter.encodeDataPacket(columns: columns, revision: 54_478)
        let parsed = try Self.parseEncodedDataPacket(dataPacket)
        let recovered: [UUIDRow] = try RawClickHouseCodableDecoder.decodeRows(
    ...
    func nullableRoundTrip() throws {
        ...
        let dataPacket = try RawClickHouseBlockWriter.encodeDataPacket(columns: columns, revision: 54_478)
        let parsed = try Self.parseEncodedDataPacket(dataPacket)
        let recovered: [NRow] = try RawClickHouseCodableDecoder.decodeRows(
            type: NRow.self, columns: parsed, rowCount: inputRows.count
        )
        #expect(recovered == inputRows)
    }

    // Parses a Data packet emitted by RawClickHouseBlockWriter and
    // reconstructs the per-column body slices in the layout the wire
    // transport delivers to RawClickHouseCodableDecoder.parseTypedColumns.
    static func parseEncodedDataPacket(_ packet: [UInt8]) throws -> [RawClickHouseNamedColumn] {
        var cursor = 0
        let (packetType, packetTypeBytes) = try packet.withUnsafeBufferPointer { buffer in
            try RawClickHouseWire.readUVarInt(base: buffer.baseAddress!, offset: cursor, limit: packet.count)
        }
        cursor += packetTypeBytes
        #expect(packetType == 2)
        let tableName = try packet.withUnsafeBufferPointer { buffer in
            try RawClickHouseWire.readString(base: buffer.baseAddress!, offset: cursor, limit: packet.count)
        }
        cursor += tableName.1
        ...
        // parser by going through `RawClickHouseBlock` synthesis.
        ...
        return try combinedBody.withUnsafeBytes { rawBuffer in
            try RawClickHouseCodableDecoder.parseTypedColumns(block: block, body: rawBuffer)
        }
    }
    ...
    private static func advanceBlockInfo(packet: [UInt8], offset: Int) throws -> Int {
        ...
        while true {
            ...
            case 1: cursor += 1
            case 2: cursor += 4
            default: throw RawClickHouseError.protocolError(stage: "blockInfo", message: "unknown field \(field)")
            }
        }
    }
    ...
    static func columnBodyLength(type: String, rowCount: Int, packet: [UInt8], offset: Int) throws -> Int {
        ...
        default:
            throw RawClickHouseError.protocolError(stage: "columnBodyLength", message: "unsupported \(type)")
        }
    }
}
