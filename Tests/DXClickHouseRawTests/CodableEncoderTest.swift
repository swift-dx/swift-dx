...
@Suite("DXClickHouseRaw Codable encoder")
struct RawClickHouseRowEncoderTest {
    ...
    func rejectsLateColumnIntroduction() throws {
        ...
        let caught = captureEncoderError {
            _ = try RawClickHouseRowEncoder().encode(rows)
        }
        switch caught {
        case .some(let error):
            switch error {
    ...
    func rejectsPlatformInt() throws {
        ...
        let caught = captureEncoderError {
            _ = try RawClickHouseRowEncoder().encode([R(x: 42)])
        }
        switch caught {
        case .some(let error):
            switch error {
    ...
    private func captureEncoderError(_ body: () throws -> Void) -> RawClickHouseError? {
        do {
            try body()
            return nil
