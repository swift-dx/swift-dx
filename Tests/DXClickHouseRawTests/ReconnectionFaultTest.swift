...
@Suite(
    "RawClickHouseConnection reconnection fault tests",
...
struct RawClickHouseReconnectionFaultTests {
    ...
    func sendAfterServerRestartReconnects() throws {
        ...
        do {
            ...
            switch error {
            case .socketIOFailed, .unexpectedEOF, .connectionFailed, .reconnectExhausted:
    ...
    private func primeConnection(connection: RawClickHouseConnection, deadlineSeconds: Double) -> Bool {
    ...
    private func waitForBroker(host: String, port: Int, timeoutSeconds: Double) {
        ...
        while Date() < deadline {
            do {
                let probe = try RawClickHouseConnection(host: host, port: port, reconnectionPolicy: .disabled)
                probe.close()
                return
            } catch {
